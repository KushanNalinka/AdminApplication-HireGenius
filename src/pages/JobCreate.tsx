...
const JobForm: React.FC = () => {
  ...
  const [preliminaryQuestions, setPreliminaryQuestions] = useState<string[]>(["", "", "", "", ""]);
  ...
  const handlePreliminaryChange = (index: number, value: string) => {
    const updatedQuestions = [...preliminaryQuestions];
    updatedQuestions[index] = value;
    setPreliminaryQuestions(updatedQuestions);
  };
      ...
      <div className="flex items-center justify-center min-h-screen bg-[#2A2438] p-6">
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
            ...
            <label className="text-[#DBD8E3] col-span-2">Job Posted Date</label>
            {/* <DatePicker
              selected={formData.jobPostedDate}
              onChange={(date: Date) => setFormData({ ...formData, jobPostedDate: date })}
              className="w-full p-3 border rounded-lg bg-[#5C5470] text-white"
            />

            <label className="text-[#DBD8E3] col-span-2">Job Ends Date</label>
            <DatePicker
              selected={formData.jobEndsDate}
              onChange={(date: Date) => setFormData({ ...formData, jobEndsDate: date })}
              className="w-full p-3 border rounded-lg bg-[#5C5470] text-white"
            /> */}
            ...
            <button
              ...
              className="bg-blue-600 text-white py-3 rounded-lg font-semibold hover:bg-blue-700 col-span-2"
            >
              Add Preliminary Questions
            </button>
          ...
          <div className="bg-[#352F44] p-6 rounded-lg shadow-lg w-1/3">
            ...
            {preliminaryQuestions.map((q, i) => (
              <input
                ...
                type="text"
                value={q}
                onChange={(e) => handlePreliminaryChange(i, e.target.value)}
                className="w-full p-2 border rounded-lg mb-2 bg-[#5C5470] text-white"
              />
            ))}
            <button

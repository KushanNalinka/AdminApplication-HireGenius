...
const JobApplicationForm = () => {
  ...
  const handleTranscriptChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) setTranscript(e.target.files[0]);
  };
  ...
  const removeExperienceCard = (id: number) => {
    setExperienceCards(experienceCards.filter((card) => card.id !== id));
  };

  const handleExperienceChange = (id: number, field: keyof ExperienceCard, value: any) => {
    const updatedExperience = experienceCards.map((card) =>
      card.id === id ? { ...card, [field]: value } : card
    );
    setExperienceCards(updatedExperience);
  ...
  const removeEducationCard = (id: number) => {
    setEducationCards(educationCards.filter((card) => card.id !== id));
  };

  const handleEducationChange = (id: number, field: keyof EducationCard, value: any) => {
    const updatedEducation = educationCards.map((card) =>
      card.id === id ? { ...card, [field]: value } : card
    );
    setEducationCards(updatedEducation);
  ...
  return (
    <div>
      {/* Your JSX content here as in your original component */}
    </div>
  );
};
